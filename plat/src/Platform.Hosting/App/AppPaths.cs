...
public class AppPaths : IAppPaths
{
    ...
    public AppPaths(AppPathsOptions? options = null, IAppInfo? environment = null)
    {
        options ??= new AppPathsOptions();
        if (options.AppDirectoryName.IsNullOrWhiteSpace())
            this.AppDirectoryName = environment?.Name ?? AppInfo.GetAssemblyName();
        else
            this.AppDirectoryName = options.AppDirectoryName;
